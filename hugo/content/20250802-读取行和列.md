---
title: "如何用pandas在Excel中生成行列以及单元格"
...
**今天，我们开始学习《30讲Python+Excel数据分析》课程的第4讲。**

这一讲是介绍如何用pandas在Excel表格中创建行和列以及单元格的功能。

你们可以想象这样一个场景，将多个CSV文件中的数据合并到一个Excel表格中，每个文件的内容分别放在不同的列或行。

相信你们遇到这样的一个场景的话，肯定不会手动操作了吧。所以，怎么办呢？


**答案是，用Python代码来实现。**

所以，基于这个场景需求，我们团队就开发了用Python代码就能实现在Excel表格中创建行和列以及单元格，然后对其进行操作的功能。
...
虽然，我们这个功能的需求很好理解，就是实现用pandas创建Excel表格中的行和列并对其进行操作的功能，但是关键是我们如何用Python代码来实现。
...
# 以字典的形式创建dataframe时，name的值就表示列名，以列表的形式创建dataframe时，name的值就表示成了行索引
...
OK，到这里我要告诉大家的是，今天这篇文章教给大家的代码的视频讲解都在《30讲Python+Excel数据分析》这套课程的第4讲内容中了，对讲解视频感兴趣的可以去本套课程中学习一下。
