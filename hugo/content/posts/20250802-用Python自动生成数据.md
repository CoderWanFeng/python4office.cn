...
**今天，我们开始学习《30讲Python+Excel数据分析》课程的第7讲。**

这一讲是介绍如何用Python代码自动生成Excel表格中的数据的功能。

你们可以想象这样一个场景，有一个表格，需要计算一个价格所在列的数据，在Excel中很好操作，但是如果数据量很大的话，那在Python中该如何实现效率最高呢？

**答案是，用Python支持的pandas库来实现。**

所以，基于这个场景需求，我们团队就开发了用Python代码就能实现自动计算excel表格数据然后对其进行填充的功能。
...
# index_col表示设置索引为编号所在的列
example = pd.read_excel(r"C:\Users\zzqyy\Desktop\Pandas玩转Excel\06函数填充，计算列\example.xlsx",index_col="编号")


# 利用series中的apply方法实现目录价格增加8元的操作
# lambda表示Python中的匿名函数
example["目录价格"] = example["目录价格"].apply(lambda x:x + 8)
...
OK，到这里我要告诉大家的是，今天这篇文章教给大家的代码的视频讲解都在《30讲Python+Excel数据分析》这套课程的第7讲内容中了，对讲解视频感兴趣的可以去本套课程中学习一下。
