---
title: 'Python PPT to Images: One-Click Export All Slides as HD Images, Supports PNG/JPG Format'
...
> **PPT to Images**: Only one line of code to convert PPT to images, with support for choosing single or multiple images.
...
```
#### Call the Function
Copy the code below, modify the file location, set whether to merge into one image, right-click and select Run
```python
...
# input_file: Path + filename of the file to convert
# output_path: Output path for converted images
# merge: When True, all PPT slides are converted to one image; When False, each slide becomes a separate image
```
