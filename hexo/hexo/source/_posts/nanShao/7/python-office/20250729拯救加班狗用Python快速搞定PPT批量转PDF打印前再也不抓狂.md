...
![20250729拯救加班狗用Python快速搞定PPT批量转PDF打印前再也不抓狂.md](https://images.unsplash.com/photo-1526379095098-d400fd0bf935?w=800&h=400&fit=crop)



大家好，我是晚枫，用通俗易懂的方式，教小白学python。

今天，我们来聊一聊如何用Python技术的手段，一行代码快速将PPT文件转换成PDF文件。

试想一下，如果你文件夹下有100个PPT的文件，你如何将它们给一键转换成PDF文件呢？

怎么样，是不是难住了？

没关系哈，我们今天要学习的这个代码就是专门为解决这个问题而开发的，而且只需要一行Python代码就可以实现。
...
虽然，我们这个功能的需求很好理解，就是批量转换PPT文件到PDF文件，但是关键是我们如何用Python代码来实现。
...
# 填入你的ppt目录
office.ppt.ppt2pdf(path=r'./test_files/50-29-ppt2pdf',output_path=r'./test_files/50-29-ppt2pdf/output')
```
参数解释：
```
path：需要转换为PDF文件的PPT文件存放的路径位置
output_path：转换之后的文件存放的位置
```
