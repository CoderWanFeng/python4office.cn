---
title: 文件太多人快疯了？用Python同时查找100个Excel文件只需0.5秒
...
![20250728文件太多人快疯了用Python同时查找100个Excel文件只需0.5秒.md - 配图1](https://images.pexels.com/photos/1181263/pexels-photo-1181263.jpeg?auto=compress&cs=tinysrgb&w=800)



大家好，我是晚枫，用通俗易懂的方式，教小白学python。

今天，我们要聊的内容是如何用Python代码实现同时查找100个Excel中的内容。
...
这个功能的需求很好理解，但是关键是我们如何用Python代码来实现。

不过，在看代码之前，我们先来体会一下具体有哪些场景下会用到这个同时查找100个Excel中的内容的功能需求。

只有明白了场景，我们才能更加有目的性地使用代码，这叫有的放矢。

比如，统计某个平台的粉丝数量。

有100个Excel文件，我们要查找并汇总出文件中包含小红书平台粉丝数量的内容。

比如，销售找历史报价。

客户突然问三年前某型号报价，销售文件夹 100 个报价单，忘记放哪；全文件秒搜型号关键词，立刻发过去，单子不丢。

比如，人事查员工考勤异常。

HR 要统计过去一年谁迟到超 10 次，100 张部门考勤表分散在各子目录；一键搜索“迟到”出现次数，自动生成名单。

总之，只要“文件太多、人工点开太慢、关键词不知道在哪张表”，就必须一次性同时搜 100 个 Excel。
...
参数解释：
```
:param query_content:你要查找的关键词
:param query_path:查找的路径
:param output_path:查找汇总之后的文件存储路径
:param output_name:查找汇总之后的文件名称
...
OK，到这里我要告诉大家的是，今天这篇文章教给大家的代码的视频讲解都在给小白的《50讲Python自动化办公》这套课程的第20讲内容中了，对讲解视频感兴趣的可以去本套课程中学习一下。
