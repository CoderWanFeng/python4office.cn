...
![20250729别再改后缀了！1行Python代码让doc-docx秒互转文件秒复活.md](https://images.unsplash.com/photo-1526379095098-d400fd0bf935?w=800&h=400&fit=crop)



大家好，我是晚枫，用通俗易懂的方式，教小白学python。

今天，我们一起来聊一聊如何用Python技术的手段，一行代码实现doc文件和docx文件互转，也就是Word文件格式转换。

有人说，直接将Word文件的后缀改一下不就行了吗？

但是，我想告诉你的是，直接改后缀的方式改出来的文件直接就不能使用了，你是不是很无语。

其实，我也很无奈，我也想改个后缀就实现Word文件doc和docx格式的互转。

那没有办法，我们只能尝试用技术的手段解决这个问题了，而且经过设计，我们仅仅用了一行代码就搞定了。
...
虽然，我们这个功能的需求很好理解，就是Word文件格式互转，但是关键是我们如何用Python代码来实现。
...
office.word.docx2doc(input_path=r'./test_files/50-43-doc2docx/程序员晚枫.docx', output_path=r'./test_files/50-43-doc2docx/out')
# office.word.doc2docx(input_path, output_path)
```
参数解释：
```
input_path：你要转换的文件路径和名称
output_path:转换之后的文件存储位置
```
...
OK，到这里我要告诉大家的是，今天这篇文章教给大家的代码的视频讲解都在给小白的《50讲Python自动化办公》这套课程的第43讲内容中了，对讲解视频感兴趣的可以去本套课程中学习一下。
