﻿---
title: 小白也能玩，1行命令批量创建文件夹，连多层子目录都帮你排好队
date: 2025-07-28 23:41:49
tags: [50讲Python自动化办公]
---
<!-- more -->

![20250728小白也能玩1行命令批量创建文件夹连多层子目录都帮你排好队.md - 配图1](https://images.pexels.com/photos/1181263/pexels-photo-1181263.jpeg?auto=compress&cs=tinysrgb&w=800)
...
今天，我们一起来聊一聊如何用Python技术的手段，一行代码快速实现文件夹的创建，甚至实现批量创建文件夹的操作。

其实，单个文件夹的创建没什么好说的，关键是如何批量创建文件夹和创建子文件夹，而且还是用一行代码实现，该怎么做呢？

大家可以先自己思考一下。

## 01

虽然，我们这个功能的需求很好理解，就是（批量）创建文件夹，但是关键是如何用Python代码来实现。
...
参数解释：
```
只有一个参数，填上你要创建文件夹的路径位置，支持创建子文件夹
```
...
OK，到这里我要告诉大家的是，今天这篇文章教给大家的代码的视频讲解都在给小白的《50讲Python自动化办公》这套课程的第32讲中了，对讲解视频感兴趣的可以去本套课程中学习一下。
