...
- **中小文件（<10万行）**：优先用**pandas**，一行代码完成多sheet读取：  
  `df = pd.read_excel("python-office_data.xlsx", sheet_name=["Sheet1", "Sheet2"])`  
...
- **数据类型转换**：读取时指定dtype，减少内存占用：  
  `pd.read_excel("python-office_data.xlsx", dtype={"ID": str, "金额": float})`  
...
- **数据分析+格式美化**：pandas处理数据，openpyxl添加样式：  
  ```python
  # pandas处理数据
  df = pd.read_excel("python-office_data.xlsx")
  df["利润"] = df["收入"] - df["支出"]
