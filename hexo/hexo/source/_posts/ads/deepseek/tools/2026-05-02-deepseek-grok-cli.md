---
title: Grok CLI 接入 DeepSeek Coding Plan 实战：5 分钟配置，AI 编程成本砍半
date: 2026-05-02 19:51:00
tags: ["Grok CLI", "DeepSeek", "Coding Plan", "xAI", "命令行", "AI编程", "程序员晚枫"]
categories: ["AI编程工具配置"]
keywords: [Grok CLI, DeepSeek Coding Plan, xAI替代, 命令行AI, 终端编程]
description: 在 Grok CLI（xAI 出的命令行 AI 助手）里接入 DeepSeek Coding Plan 完整教程：5 分钟配置步骤 + 4 个常见问题。
...
>
> 想看完整的大厂 AI Coding Plan + Token 价格对比、隐藏购买渠道、避坑提醒？这份持续更新的价格汇总表全整理好了。建议先收藏，再回来按本文 5 分钟配好 Grok CLI + DeepSeek。

---

## 一句话结论

**在 Grok CLI（xAI 出的命令行 AI 助手）里接入 DeepSeek Coding Plan，5 分钟搞定，AI 编程成本砍掉 50%+。**
- 已经是 Grok CLI 用户：**按本文 3 步配置**，换底层模型
- 喜欢 xAI 习惯：**保持 Grok CLI 交互**，换便宜模型
...
Grok CLI 是 xAI 官方推出的命令行 AI 助手，**默认绑定 Grok 模型，国内访问困难**。
**接入 DeepSeek Coding Plan 之后**：
- ✅ 中文编程能力更强（DeepSeek 比 Grok 中文强）
- ✅ 国内节点直连，响应稳定
- ✅ API 1 元/百万 tokens 起，**比 xAI API 便宜 70%+**
- ✅ 仍用熟悉的 Grok CLI 命令行交互

---

## 一、Grok CLI 接入 DeepSeek 的 4 大优势
...
### Step 2：在 Grok CLI 中配置 DeepSeek（环境变量方式）
...
```bash
echo 'export GROK_API_KEY="你的DeepSeek API Key"' >> ~/.zshrc
echo 'export GROK_API_BASE="https://api.deepseek.com/v1"' >> ~/.zshrc
source ~/.zshrc
```
...
## 三、成本对比：Grok CLI 原生 vs Grok CLI + DeepSeek

**假设每天 100 次 AI 编程对话，每次 2000 tokens（输入 + 输出）：**

| 方案 | 月调用量 | 月成本 | 年成本 |
|------|----------|--------|--------|
| Grok CLI + Grok | 60 万 tokens | **~50 元** | ~600 元 |
| Grok CLI + DeepSeek V3.2 | 60 万 tokens | **~1 元** | ~12 元 |
| Grok CLI + DeepSeek V4 Pro（2.5 折）| 60 万 tokens | **~3 元** | ~36 元 |
...
### Q1：Grok CLI 的所有功能都能用 DeepSeek 吗？

**A：能用。** Grok CLI 是 OpenAI 兼容的 CLI 工具，**所有功能照常用**，只是底层模型换成 DeepSeek。

### Q2：会不会比 Grok 弱？

**A：分场景。**

| 任务 | DeepSeek V4 Pro | Grok | 选谁 |
|------|----------------|------|------|
| 中文代码生成 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | DeepSeek |
| 算法题 / 推理 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | DeepSeek 略胜 |
| 复杂业务逻辑 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | 持平 |
...
**结论**：编程任务 DeepSeek 全面占优，**只有实时联网 Grok 强**。

### Q3：环境变量配完不生效怎么办？

**A：3 步排查**：
1. **重启终端**：环境变量修改后必须重启终端
2. **检查变量名**：必须是 `GROK_API_KEY` 和 `GROK_API_BASE`（不是 `DEEPSEEK_API_KEY`）
3. **检查 URL 末尾**：必须是 `https://api.deepseek.com/v1`（带 `/v1`）

### Q4：能同时用 Grok 和 DeepSeek 吗？
...
export GROK_API_KEY="你的Grok Key"

# 切到 DeepSeek
export GROK_API_BASE="https://api.deepseek.com/v1"
export GROK_API_KEY="你的DeepSeek Key"
```

---

## 五、Grok CLI vs 其他编辑器 + DeepSeek
...
**一句话：在 Grok CLI 里用 DeepSeek，配置 5 分钟，省钱 90%+，中文还更好。**
