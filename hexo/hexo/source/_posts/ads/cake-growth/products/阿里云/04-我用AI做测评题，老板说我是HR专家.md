---
title: 04-我用AI做测评题，老板说我是HR专家
date: 2026-05-21 00:46:00
tags: ["公众号文章", "AI工具", "阿里云SBTI"]
categories: ["公众号文章", "Python自动化办公社区"]
cover: https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?q=80&w=1200&auto=format&fit=crop
---

> 大家好，我是程序员晚枫。

我有个朋友，是创业公司的HR，每次招聘都要出测评题。

他问我："晚枫，我要给候选人出测评题，但不知道怎么出。"

我说："用AI啊，5分钟搞定。"

他不信。

**"AI能出测评题？"**

是的，而且比大多数HR出的还专业。

---

## 我不是HR，但我需要做测评

你们有没有这种感觉？

招聘要做测评、培训要做测评、考核要做测评...

但不知道怎么出题、怎么评分、怎么分析。

**你又不是HR专家，为什么要花那么多时间？**

---

## 我试了5款AI测评工具，这款最专业

上个月我帮朋友做招聘测评，开始测试各种AI测评工具。

试了北森、Moka、智员云...

要么太贵，要么太复杂，要么功能不全。

直到遇到**阿里云SBTI**。

**它是阿里云的AI测评平台，帮你快速出题、分析、生成报告。**

我帮朋友输入岗位要求，AI帮他生成了完整的测评方案。

题目、评分标准、候选人分析报告，一条龙。

**以前3天，现在2小时。**

---

## 我用SBTI做了什么？

**1. 招聘测评**

朋友公司招聘，需要给候选人做测评。

用SBTI，输入岗位要求，AI生成测评题目。

候选人做完，AI自动评分，生成分析报告。

**以前要请HR专家，现在AI就搞定了。**

**2. 培训测评**

公司做完培训，要测评员工学习效果。

用SBTI，输入培训内容，AI生成测评题目。

自动评分，自动生成培训报告。

**以前1周，现在1小时。**

**3. 绩效考核**

年底要做绩效考核，要给员工打分。

用SBTI，输入考核指标，AI帮你打分。

自动生成绩效报告，告诉你谁表现好、谁需要改进。

**以前要开很多会，现在10分钟出结果。**

---

💬 **来评论区聊聊**

你们公司做什么测评？评论区聊聊。

如果你也想用AI做测评，试试阿里云SBTI。

👉 [点击这里试试SBTI：https://www.aliyun.com/solution/tech-solution/sbti?source=5176.29345612&userCode=t6duaoe1](https://www.aliyun.com/solution/tech-solution/sbti?source=5176.29345612&userCode=t6duaoe1)

---

## 3条建议，帮你用AI做测评

**1. 测评要有目的**

很多人做测评，不知道要测评什么。

**先想清楚你的目的，再选择测评工具。**

SBTI帮你根据目的生成合适的测评题目。

**2. 不要只看分数**

很多人做测评，只看分数。

**分数只是参考，要看背后的分析。**

SBTI帮你生成详细分析报告，告诉你为什么是这个分数。

**3. 用AI省时间**

做测评最费时间的是出题和分析。

AI帮你出题、分析、生成报告，省下大量时间。

**你只需要审核和决策。**

---

## 最后说句掏心窝子的

我以前觉得做测评是HR的事。

**但SBTI让我发现：每个人都可以做专业测评。**

你不需要是HR专家，不需要花大量时间。

你只需要一个工具，然后让AI帮你做。

**2小时，一个测评方案。**

就这么简单。

---

## 相关阅读

- [2026 年学习编程，你需要的不是更多教程](https://mp.weixin.qq.com/s/YS0shsl6vJD_wUzow7NOnw)
- [人在曼谷旅游，AI在帮我赚钱](https://mp.weixin.qq.com/s/KLXXEoxMu9uayJTLXStIzw)
- [设计师花3天做的图，我用 AI 15分钟搞定了](https://mp.weixin.qq.com/s/BQZUEFVCWhx8lLDaQsbRTg)
- [Codex 入门指南，从零基础到实战，看这一篇就够了！](https://mp.weixin.qq.com/s/ilvNENMEiPy2uEYyDflvQA)
- [比AI更可怕的是：你同事已经开始用AI工作了](https://mp.weixin.qq.com/s/__84ENDwkz2zVTdvKH5h4A)
- [高考后上大学，普通人别选AI专业](https://mp.weixin.qq.com/s/AtgtbGpaueW58olyO7sDrw)

💬 **来评论区聊聊**

你们公司做什么测评？有什么心得？评论区聊聊。

还没试过的朋友，点下面的链接试试阿里云SBTI。

👉 [点击这里试试SBTI：https://www.aliyun.com/solution/tech-solution/sbti?source=5176.29345612&userCode=t6duaoe1](https://www.aliyun.com/solution/tech-solution/sbti?source=5176.29345612&userCode=t6duaoe1)

---

---

**科技不高冷，AI很好用。**

我是晚枫，关注我，带你一起玩AI！